...
class LoanEngine:
    ...
    def buyoff_loan(self, individual_id, loan_ref, date_str=None):
        """fully settle a loan including Principal + Accrued Interest + Unearned Future Interest."""
        loan = self.db.get_loan_by_ref(individual_id, loan_ref)
        if not loan or loan['status'] != 'Active':
            return False
        ...
        accrued_interest = loan.get('interest_balance', 0.0)
        future_interest = loan.get('unearned_interest', 0.0)
        
        total_payoff = principal + accrued_interest + future_interest
        
        # We perform a "Repayment" for the full amount
        # But we must tag it as "Loan Buyoff" for clarity?
        # Or just "Repayment" with a note?
        # Let's use event_type="Repayment" so it counts as deduction, 
        # but the note will say "Loan Buyoff".
        
        # Wait, if we use "Repayment", does it handle unearned interest correctly?
        # Standard "Repayment" logic (in recalculate_balances) might assume 
        # we are paying off Accrued first, then Principal. 
        # It doesn't know about "Unearned Interest" being paid.
        # If we pay 'future_interest', where does it go?
        # In our ledger, 'deducted' reduces total balance.
        # But 'unearned_interest' is NOT in the ledger balance traditionally 
        # (ledger balance = Principal + Accrued).
        # Unearned is off-book.
        
        # So, if we pay Future Interest, we are paying EXTRA money that wasn't in the ledger balance yet.
        # This means the "Total Bal" (Gross) reduces correctly (since it included unearned).
        # BUT the internal "Balance" (Net) will become negative if we pay more than P + Accrued?
        
        # Let's think:
        # Net Balance = Principal + Accrued.
        # We are paying: Principal + Accrued + Future.
        # If we deduct (P+A+F), Net Balance becomes -F.
        # This implies we "overpaid" the account.
        
        # BUT, if we are paying off future interest, that interest effectively becomes "Earned" immediately upon buyoff?
        # Yes. If the deal is "inclusive of future interest", we are converting Unearned -> Earned.
        
        # So we should convert Unearned to Earned first?
        # 1. Add "Interest Earned" transaction for the `future_interest` amount.
        #    This increases Net Balance by `future_interest`.
        #    Net Balance becomes P + Accrued + Future.
        # 2. Then Pay `total_payoff`.
        #    Net Balance becomes 0.
        
        # EXACTLY.
        
        # Step 1: Realize Userned Interest
        if future_interest > 0:
            tr_note = f"Buyoff: Realizing unearned interest"
            # We add a transaction to realize it?
            # Or just update the opening balance?
            # Better to add a transaction "Interest Earned" (or "Interest Adjustment").
            
            # We need to know previous balances for the transaction record
            df = self.get_ledger_df(individual_id)
            last_bal = df["balance"].iloc[-1] if not df.empty else 0.0
            last_i_bal = df["interest_balance"].iloc[-1] if not df.empty else 0.0
            
            new_bal = last_bal + future_interest
            new_i_bal = last_i_bal + future_interest
            
            self.db.add_transaction(
                individual_id, date_str, "Interest Earned", loan_ref,
                future_interest, 0, new_bal,
                tr_note,
                interest_amount=future_interest,
                interest_balance=new_i_bal,
                principal_balance=loan.get('principal_balance', 0) # Unchanged? Actually we rely on recalculate mostly
            )
            # FORCE recalculate to ensure state is clean before next step?
            # self.recalculate_balances(individual_id)
            # Actually, let's just use add_loan_event logic? No, specific manual add is fine.
            
        # Step 2: Pay Off Everything
        # Now Net Balance should be target amount.
        
        note = f"Loan Buyoff: Full Settlement (P: {principal:,.0f}, I: {accrued_interest+future_interest:,.0f})"
        
        # We can use add_transaction directly.
        # We rely on recalculate_balances to sort out the final 0.
        
        # We need to explicitly clear the Loan Record too.
        # status="Paid", balance=0, interest=0, unearned=0
        
        # But wait, we should let recalculate_balances handle the ledger math.
        # We just add the transaction.
        
        # Update Loan Status first?
        self.db.update_loan_status(loan['id'], 0, loan['next_due_date'], "Paid", interest_balance=0, unearned_interest=0)
        
        # Add Repayment Transaction
        # We need to fetch balances again if we added interest?
        # Actually, self.recalculate_balances(individual_id) will fix everything regardless of what we write in snapshot.
        # But let's try to be accurate.
        
        # If we added 'Interest Earned', the next transaction follows it.
        
        self.db.add_transaction(
            individual_id, date_str, "Loan Buyoff", loan_ref,
            0, total_payoff, 0, # Balance will be fixed by recalc
            note,
            principal_portion=principal,
            interest_portion=accrued_interest + future_interest
        )
